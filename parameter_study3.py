...
for size in [4,8, 32, 64, 128, 256]:
    ...
    embfile = open( "./data/vfest_%d.poincare"%size, 'r') 
    ...
    result_file= open('./emb_vectors/result_poincare%d.txt'%size, 'w') 
      
    for i in range(len(accuracies)):
        
        print(i,'accuracy %.3f'%accuracies[i], 'precisions %.3f' % precisions[i], 'recalls %.3f'%recalls[i], 'fscores %.3f' %fscores[i])   
    ...
    result_file.write('mean accuracy '+ str(np.mean(accuracies)))    
    result_file.write('\n')
